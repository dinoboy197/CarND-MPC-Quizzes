...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; ++t) {
      // add cost based on cte
      fg[0] += CppAD::pow(vars[cte_start + t], 2);
      // add cost based on epsi
      fg[0] += CppAD::pow(vars[epsi_start + t], 2);
      // add cost based on difference between current and target velocity
    ...
    for (int t = 1; t < N; t++) {
      ...
      fg[1 + epsi_start + t] = epsi1 - (psi0 - CppAD::atan(coeffs[1]) + dt * delta0 * v0 / Lf);
    }
  }
};
